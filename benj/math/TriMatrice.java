...
public class TriMatrice<T> extends BaseMatrice<T> {
	private final Object[][] mat;
	private final Class<T> clazz;
	
	public TriMatrice(int n, Class<T> clazz) {
	...
	@Override
	protected Matrice<T> safeSet(int i, int j, T value) {
		Pair<Integer,Integer> pos = getRealPos(i,j);
	...
	protected void checkInBounds(int i, int j) {
		if (i < 0 || mat.length <= i || j < 0 || mat.length <= j)
